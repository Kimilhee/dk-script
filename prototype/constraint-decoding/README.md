...
펜을 뗀 뒤 250ms가 지나면 `problem` 모드 하나를 세 개 beam으로 디코딩한다. `4모드 비교`는 기본 결과부터 각 모드가 끝나는 즉시 하나씩 표시한다. 화면의 `전체 Nms`가 캡처부터 결과 수신까지의 end-to-end 지연이다. 목표는 P580에서 200ms 이하지만, 현재 공개 baseline으로 달성됐다고 간주하지 않는다.
