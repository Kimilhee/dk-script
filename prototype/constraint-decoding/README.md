...
초등·중등·고등 중 하나를 고르고 예제를 따라 쓴 뒤 `변환`을 누른다. 화면의 시간은 버튼을 누른 시점부터 인식 결과를 받은 시점까지이며 모델 초기화와 화면 렌더링 시간은 포함하지 않는다. P580 실기기에서 확인한 수치는 아니며, 목표는 최종 입력 후 p50 200ms 이하이다.

결과와 대안 후보는 KaTeX로 렌더링하며 원본 LaTeX는 요소의 `title`에 남긴다. 결과가 예제와 일치하면 정답으로 표시하고 다음 수식으로 갈지 묻는다. 학교급별 예제는 `src/practice.ts`에서 관리한다.

P580 세로 화면에서는 페이지 스크롤 없이 캔버스가 남은 높이를 사용한다.
