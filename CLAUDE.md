...
### P580 실측 (2026-09-22) — 가정하지 말고 이 값을 써라

adb + Chrome DevTools Protocol로 실기기에서 확인했다. 추정치가 아니다.

| 항목                    | 값                                          | 이게 닫아버리는 선택지                                                 |
| ----------------------- | ------------------------------------------- | ---------------------------------------------------------------------- |
| Android / Chrome        | 8.1.0 / **138.0.7204.179**                  | Android 8 지원 마지막 Chrome. 브라우저 업그레이드로 얻을 건 없다       |
| CPU ABI                 | **armeabi-v7a (32비트)**                    | arm64 아님. WASM JIT·SIMD 로워링이 arm64보다 불리하다                  |
| `hardwareConcurrency`   | 8 (`/proc/cpuinfo`는 4 — 나머지는 핫플러그) |                                                                        |
| `deviceMemory`          | 2                                           | 워커를 2개 띄우면 앱이 얼어붙는다. 측정용 세션도 앱 워커를 재사용할 것 |
| **WASM SIMD**           | **지원됨 (이미 활성)**                      | "SIMD 폴백이면 2~4× 이득" 카드는 **없다**                              |
| **`SharedArrayBuffer`** | **없음** (`crossOriginIsolated: false`)     | **스레드 불가.** `numThreads`를 올려도 무의미                          |
| WebGPU                  | 없다고 가정                                 |                                                                        |

`numThreads = 1`은 하드코딩 실수가 아니라 환경이 강제한 값이다. 살리려면 `public/sw.js`에서 COOP/COEP를 주입해야 하는데(GitHub Pages는 헤더를 못 넣는다), COEP를 켜면 교차 출처 모델 로딩이 깨진다. 모델 자체 호스팅이 전제다.

### P580 실측 성능 (Hand-to-TeX baseline, beamSize 4)

배포된 앱의 워커를 그대로 써서 측정했다. 같은 필기를 50Hz/240Hz 두 밀도로 넣었다.

| 수식                | 출력 토큰 |   전체 | **decode** | encode | decode 비중 |
| ------------------- | --------: | -----: | ---------: | -----: | ----------: |
| `1/x≈1-x`           |         7 | 2262ms | **1519ms** |  743ms |         67% |
| `abca⁻¹b⁻¹d∼?`      |        16 | 3747ms | **3157ms** |  591ms |         84% |
| `⇔\|PA\|²=9\|PB\|²` |         8 | 2842ms | **1866ms** |  976ms |         66% |

읽어야 할 세 가지:

1. **리샘플링 검증 완료.** 원시 입력 점이 206 → 3723으로 18배 차이 나도 전체 시간이 평평했고, encode는 591~976ms로 입력 점 수와 무관했다. 입력 장치 샘플레이트가 제거됐다는 직접 증거다.
2. **비용은 입력 길이가 아니라 출력 토큰 수에 붙는다.** 출력 토큰 1개당 약 200~230ms. 답안이 짧은 우리 도메인에는 유리한 성질이다.
3. **병목은 디코더(66~84%)로 옮겨갔다.** 리샘플을 더 조여도 얻을 게 거의 없다. 다음 레버는 `beam.ts`의 빔 순차 실행을 batch 축으로 묶는 것 하나다 — 토큰마다 `decoder.run()`을 beamSize 번 부르고 그때마다 `mem_k`/`mem_v`를 WASM 힙으로 재복사한다.
